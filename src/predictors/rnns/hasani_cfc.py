...
class Cfc(nn.Module):
    ...
    def forward(self, x, timespans=None, mask=None):
        ...
        for t in range(seq_len):
            ...
            ts = torch.full((batch_size,), self.dt, device=device)
            if mask is not None:
...
class HasaniCfC(Predictor):
    def __init__(
        self, input_size: int, num_classes: int, hidden_size: int = 32
    ):
        super(HasaniCfC, self).__init__()
